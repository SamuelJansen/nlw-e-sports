...
const getInput = (input: InputProps) => {
    const classNameAsString: string = `${resolveWidth(input.width)} h-full bg-zinc-900 py-3 px-4 rounded text-sm placeholder:text-zinc-500`
    return (
        <input 
            key={input.id}
            {...input}
...
export const InputSelect = (props: InpusSelectProps) => {
    ...
    return (
        <div className={classNameAsString}>
            <label htmlFor={props.elements[0].id}>{props.label}</label>
